...
const DETAILS_API = "http://localhost:8000/api/about-founder-details";
const SECTION_API = "http://localhost:8000/api/about-founder";
...
const DEFAULT_SECTION = {
  heroSubtext: "Sacred Lineage of",
  heroTitle: "Our Revered Founders",
  heroDescription:
    "Enlightened masters guiding seekers on the path of self-realization and inner peace",
  videoLabel: "Moments of Wisdom",
  ...
  footerButton: "Start Your Journey Today",
  footerIcon: "",
};
...
const toList = (value) => {
  ...
  const byLine = asString.split(/\r?\n/).map((v) => v.trim()).filter(Boolean);
  if (byLine.length > 1) return byLine;
  const byPipe = asString.split("|").map((v) => v.trim()).filter(Boolean);
...
const parseStatLine = (line) => {
  ...
  for (const sep of separators) {
    if (trimmed.includes(sep)) {
      const [number, ...rest] = trimmed.split(sep);
      return {
        number: number.trim(),
        label: rest.join(sep).trim(),
      };
    }
  }
  ...
  if (Array.isArray(value)) {
    return value
      .map((item) => {
        ...
        return parseStatLine(item);
      })
      .filter((item) => item && (item.number || item.label));
  }
  const parsed = safeJsonParse(value);
  if (Array.isArray(parsed)) return toStats(parsed);
  const asString = String(value);
  const byLine = asString.split(/\r?\n/).map((v) => v.trim()).filter(Boolean);
  const lines = byLine.length ? byLine : [asString.trim()];
  return lines
    .map(parseStatLine)
    .filter((item) => item && (item.number || item.label));
};
...
const toEmbedUrl = (url) => {
  ...
  try {
    ...
    if (parsed.hostname.includes("youtube.com")) {
      ...
      if (id) {
        return `https://www.youtube.com/embed/${id}?controls=0&modestbranding=1`;
      }
    }
...
const splitHighlight = (title) => {
  const text = (title || "").trim();
  if (!text) return { prefix: "", highlight: "" };
  if (text.includes("||")) {
    const [prefix, highlight] = text.split("||");
    return { prefix: prefix.trim(), highlight: (highlight || "").trim() };
  }
  const words = text.split(/\s+/);
  if (words.length === 1) return { prefix: "", highlight: words[0] };
  return {
...
const normalizeFounder = (item, index) => {
  const philosophySource =
    Array.isArray(item?.philosophyItems) && item.philosophyItems.length
      ? item.philosophyItems
      : Array.isArray(item?.philosophy)
        ? item.philosophy
        : [];

  const philosophy = philosophySource
    .map((p) => {
      const rawIcon = p?.icon || "";
      const icon =
        rawIcon && (rawIcon.includes("/") || rawIcon.includes("."))
          ? toAssetUrl(rawIcon)
          : rawIcon;
      return {
        icon,
    ...
    .filter((p) => p.icon || p.title || p.description);

  return {
    id: item?._id || item?.id || index,
    name: item?.founderName || item?.name || "",
    role: toList(item?.role),
    image: toAssetUrl(item?.img || item?.image || ""),
    shortTitle: item?.shortTitle || "",
    bio: toParagraphs(item?.mainBiography || item?.bio),
    philosophy,
    achievements: toList(item?.mainAchievements || item?.achievements),
    quote: {
      text: item?.mainQuote || item?.quote?.text || "",
      author: item?.quoteTitle || item?.quote?.author || "",
    },
    stats: toStats(item?.stats),
  };
};

const normalizeFounders = (items) => {
  if (!Array.isArray(items)) return [];
  return items
    .map((item, index) => normalizeFounder(item, index))
    .filter((founder) => founder.name);
};

const normalizeSection = (items) => {
  const section = Array.isArray(items) && items.length ? items[0] : null;
  if (!section) return null;
  return {
    heroSubtext: section.journeyHeading || DEFAULT_SECTION.heroSubtext,
    heroTitle: section.journeyTitle || DEFAULT_SECTION.heroTitle,
    heroDescription: section.journeyParagraph || DEFAULT_SECTION.heroDescription,
    videoLabel: section.videoLabel || DEFAULT_SECTION.videoLabel,
    videoTitle: section.videoTitle || DEFAULT_SECTION.videoTitle,
    footerHeading: section.lastHeading || DEFAULT_SECTION.footerHeading,
    footerParagraph: section.lastParagraph || DEFAULT_SECTION.footerParagraph,
    footerButton: section.footerButton || DEFAULT_SECTION.footerButton,
    footerIcon: toAssetUrl(section.lastIcon || ""),
  };
};
  ...
  return section.videos
    .map((video, index) => {
      ...
      return {
        ...
        type,
        url,
        embedUrl: type === "link" ? toEmbedUrl(url) : url,
...
const Founders = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [detailsRes, sectionRes] = await Promise.all([
        ...
        const normalizedFounders = normalizeFounders(detailsRes.data?.data);
        if (normalizedFounders.length) {
          setFounders(normalizedFounders);
        }

        const sectionItems = sectionRes.data?.data || [];
        const normalizedSection = normalizeSection(sectionItems);
        if (normalizedSection) {
          setSectionContent((prev) => ({ ...prev, ...normalizedSection }));
        }

        const normalizedVideos = normalizeVideos(sectionItems);
        if (normalizedVideos.length) {
          setVideos(normalizedVideos);
        }
      } catch (error) {
        console.error("Failed to load About Founder data", error);
      }
    };
  ...
  useEffect(() => {
    if (activeFounder >= founders.length) {
      setActiveFounder(0);
    }
  }, [activeFounder, founders.length]);
  ...
  const currentFounder = founders[activeFounder] || founders[0] || fallbackFounder;
  const heroTitleParts = splitHighlight(sectionContent.heroTitle);
    ...
    <div className={styles.container}>
      ...
      <section className={styles.heroSection}>
        ...
        <div className={styles.heroContent}>
          ...
          <p className={styles.heroSubtext}>{sectionContent.heroSubtext}</p>
          <h1 className={styles.heroTitle}>
            {heroTitleParts.prefix}
            {heroTitleParts.prefix && heroTitleParts.highlight ? " " : ""}
            {heroTitleParts.highlight ? (
              <span className={styles.highlight}>{heroTitleParts.highlight}</span>
            ) : null}
          </h1>
      ...
      <section className={styles.founderProfile}>
        <div className={styles.profileContainer}>
          ...
          <div className={styles.profileHeader}>
            ...
            <div className={styles.profileInfo}>
              <div className={styles.founderTags}>
                {currentFounder.role.map((tag, i) => (
                  <span key={i} className={styles.tag}>
                    {tag}
                  </span>
                ))}
              </div>
              <h2 className={styles.founderName}>{currentFounder.name}</h2>
              <p className={styles.founderTitle}>{currentFounder.shortTitle}</p>
          ...
          <div className={styles.quoteSection}>
            ...
            <p className={styles.quoteText}>{currentFounder.quote.text}</p>
            <p className={styles.quoteAuthor}>— {currentFounder.quote.author}</p>
          </div>
          ...
          <div className={styles.philosophySection}>
            ...
            <div className={styles.philosophyGrid}>
              {currentFounder.philosophy.map((phil, i) => (
                <div key={i} className={styles.philosophyCard}>
                  <div className={styles.philIcon}>
                    {phil.icon && (phil.icon.startsWith("http") || phil.icon.startsWith("/")) ? (
                      <img src={phil.icon} alt={phil.title} />
                    ) : (
                      phil.icon
                    )}
                  </div>
      ...
      <section className={styles.videoSection}>
        <div className={styles.videoHeader}>
          <span className={styles.videoLabel}>{sectionContent.videoLabel}</span>
          <h2 className={styles.videoTitle}>{sectionContent.videoTitle}</h2>
      ...
      <section className={styles.footerCTA}>
        <div className={styles.footerContent}>
          {sectionContent.footerIcon ? (
            <img
              src={sectionContent.footerIcon}
              alt="Footer icon"
          ...
          <h3>{sectionContent.footerHeading}</h3>
          <p>{sectionContent.footerParagraph}</p>
          <button className={styles.footerBtn}>{sectionContent.footerButton}</button>
        </div>
      </section>
    </div>
  );
